...
> Text-only Markdown conversion of the [source PDF](../pdf/aurora-sigmod-2017.pdf). Images and diagrams are omitted; the PDF remains authoritative.
...
most notably by pushing redo processing to a multi-tenant scaleout storage service, purpose-built for Aurora. We describe how
...
_SIGMOD’17, May 14 – 19, 2017, Chicago, IL, USA._
...
such as 2-phase commit (2PC) [3][4][5] is challenging in a cloudscale distributed system. These protocols are intolerant of failure
...
approaches. First, by building storage as an independent faulttolerant and self-healing service across multiple data-centers, we
...
observe that a failure in durability can be modeled as a longlasting availability event, and an availability event can be modeled
...
1. How to reason about durability at cloud scale and how
to design quorum systems that are resilient to correlated
failures. (Section 2).
2. How to leverage smart storage by offloading the lower
quarter of a traditional database to this tier. (Section 3).
3. How to eliminate multi-phase synchronization, crash
...
operation must respectively obtain a read quorum of V r votes or a
write quorum of V w votes. To achieve consistency, the quorums
must obey two rules. First, each read must be aware of the most
recent write, formulated as V r + V w - V. This rule ensures the set
...
recent write to avoid conflicting writes, formulated as V w - V/2.
...
(V w = 2) and a read quorum of 2/3 (V r = 2).
...
model with 6 votes (V = 6), a write quorum of 4/6 (V w = 4), and a
read quorum of 3/6 (V r = 3). With such a model, we can (a) lose a
...
deployment requiring rollback. One that can handle a multisecond loss of availability of a member of a quorum can handle a
...
Amazon Simple Storage Service (S3) in order to support point-intime restores, the modified data pages, a second temporary write
...
scratch on demand every time. Note that background
materialization is entirely optional from the perspective of
correctness: as far as the engine is concerned, _the log is the_
_database_, and any pages that the storage system materializes are
...
embarrassingly parallel fashion without impacting write
...
log records in question durable or _hardened_ . The replicas use the
...
we must truncate at 1000. We are _complete_ to 1007, but only
_durable_ to 1000.
...
2. Each mini-transaction is composed of multiple
...
the normal/forward path, as the database receives
...
disk, the database establishes a _read-point_, representing the VDL
at the time the request was issued. The database can then select a
storage node that is _complete_ with respect to the read point,
...
a cluster are in a single geographical region (e.g., us-east-1, uswest-1 etc.), are typically placed in different AZs, and connect to
...
_6.1.1_ _Scaling with instance sizes_
...
_6.1.2_ _Throughput with varying data sizes_
...
_6.1.3_ _Scaling with user connections_
In this experiment, we report that throughput in Aurora can scale
...
_6.1.4_ _Scaling with Replicas_
...
_6.1.5_ _Throughput with hot row contention_
...
_6.2.1_ _Application response time with Aurora_
...
_6.2.2_ _Statement Latencies with Aurora_
...
MySQL to Aurora. The median (P50) and 95 [th] percentile (P99)
...
_6.2.3_ _Replica Lag with Multiple Replicas_
...
weeks or so. Therefore, we recently released a new ZeroDowntime Patch (ZDP) feature that allows us to patch a customer
...
network partitions. These results and our experience with cloudscale complex and correlated failures motivated our consistency
...
the associated work in LLAMA [34] and Bw-Tree [35] use logstructured techniques in multiple ways across the storage engine
...
[1] [B. Calder, J. Wang, et al. Windows Azure storage: A highly](http://doi.acm.org/10.1145/161468.16147)
...
I/O for recovery and degraded reads. In _FAST 2012._

[3] P.A. Bernstein, V. Hadzilacos, and N. Goodman.

Concurrency control and recovery in database systems,
Chapter 7, Addison Wesley Publishing Company, ISBN 0201-10715-5, 1997.

[4] C. Mohan, B. Lindsay, and R. Obermarck. Transaction

management in the R* distributed database management
system”. _ACM TODS,_ 11(4):378-396, 1986.

[5] C. Mohan and B. Lindsay. Efficient commit protocols for the

tree of processes model of distributed transactions. _ACM_
_SIGOPS Operating Systems Review_, 19(2):40-52, 1985.

[6] D.K. Gifford. Weighted voting for replicated data. In _SOSP_
...
fine-granularity locking and partial rollbacks using writeahead logging. _ACM TODS_, 17 (1): 94–162, 1992.

[8] R. van Renesse and F. Schneider. Chain replication for

supporting high throughput and availability. In _OSDI 2004_ .

[9] A. Kopytov. Sysbench Manual. Available at

                -                 http://imysql.com/wp [content/uploads/2014/10/sysbench](http://imysql.com/wp-content/uploads/2014/10/sysbench-manual.pdf)
[manual.pdf](http://imysql.com/wp-content/uploads/2014/10/sysbench-manual.pdf)
...
virtues and limitations. In _VLDB_ 2014.

[13] R. Taft, E. Mansour, M. Serafini, J. Duggan, A.J. Elmore, A.

Aboulnaga, A. Pavlo, and M. Stonebraker. E-Store: finegrained elastic partitioning for distributed transaction
processing systems. In _VLDB 2015._

[14] R. Woollen. The internal design of salesforce.com’s multi
tenant architecture. In _SoCC 2010_ .

[15] S. Davidson, H. Garcia-Molina, and D. Skeen. Consistency

in partitioned networks. _ACM CSUR_, 17(3):341–370, 1985.

[16] S. Gilbert and N. Lynch. Brewer’s conjecture and the

feasibility of consistent, available, partition-tolerant web
services. _SIGACT News_, 33(2):51–59, 2002.

[17] D.J. Abadi. Consistency tradeoffs in modern distributed

database system design: CAP is only part of the story. _IEEE_
_Computer_, 45(2), 2012.
...
[19] Y. Saito and M. Shapiro. Optimistic replication. _ACM_

_Comput. Surv._, 37(1), Mar. 2005.
...
limitations, extensions, and beyond. _ACM Queue_, 11(3),
March 2013.
...
B-tree for new hardware platforms. In _ICDE 2013_ .

[36] M. Aguilera, J. Leners, and M. Walfish. Yesquel: scalable

SQL storage for web applications. In _SOSP 2015._

[37] Percona Lab. TPC-C Benchmark over MySQL. Available at

                    -                    [https://github.com/Percona](https://github.com/Percona-Lab/tpcc-mysql) Lab/tpcc mysql

[38] P. Bernstein, C. Reid, and S. Das. Hyder – A transactional

record manager for shared flash. In _CIDR 2011_ .
...
fleet. Pinterest Engineering Blog. Available at:
                                ...
                                -                                 [https://engineering.pinterest.com/blog/sharding](https://engineering.pinterest.com/blog/sharding-pinterest-how-we-scaled-our-mysql-fleet) pinterest
[how-we-scaled-our-mysql-fleet](https://engineering.pinterest.com/blog/sharding-pinterest-how-we-scaled-our-mysql-fleet)

[41] G. Graefe. Instant recovery for data center savings. _ACM_

_SIGMOD Record_ . 44(2):29-34, 2015.
